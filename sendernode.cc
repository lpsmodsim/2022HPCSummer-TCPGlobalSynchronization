...
sendernode::sendernode( SST::ComponentId_t id, SST::Params& params ) : SST::Component(id) {
    ...
    // Stats
    sendingRateStat = registerStatistic<int>("sendingRateStat");


    registerClock(clock, new SST::Clock::Handler<sendernode>(this, &sendernode::tick));
    port = configureLink("port", new SST::Event::Handler<sendernode>(this, &sendernode::eventHandler));
    if (!port) {
...
bool sendernode::tick( SST::Cycle_t currentCycle ) { 
    if (currentCycle >= starting_cycle) {
        ...
        // Statistic Info
        sendingRateStat->addData(curr_send_rate);
        //^^^std::cout << node_id << ":" << curr_send_rate << std::endl;
        performStatisticOutput(sendingRateStat);
...
void sendernode::eventHandler(SST::Event *ev) {
...
void sendernode::sendPacket(int id, int delay) {
